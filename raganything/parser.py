...
class MineruParser(Parser):
    ...
    @classmethod
    def _pick_json_file(cls, directory: Path, file_stem: str) -> Optional[Path]:
        """Return the best available content-list JSON file in *directory*.

        MinerU 2.x introduced ``content_list_v2.json`` which contains richer
        structured data (bounding boxes, confidence scores, etc.).  When both
        files exist we prefer the v2 format; otherwise we fall back to the
        legacy ``<stem>_content_list.json``.

        Returns ``None`` when neither file exists.
        """
        v2 = directory / "content_list_v2.json"
        if v2.exists():
            cls.logger.debug(f"Using MinerU v2 content list: {v2}")
            return v2
        legacy = directory / f"{file_stem}_content_list.json"
        if legacy.exists():
            return legacy
        return None
    ...
    def _read_output_files(
        ...
        Prefers ``content_list_v2.json`` (MinerU 2.x) when available, falling
        back to the legacy ``<stem>_content_list.json`` format.
        ...
        json_file = cls._pick_json_file(output_dir, file_stem) or (
        ...
        if file_stem_subdir.is_dir():
            ...
            for subdir in file_stem_subdir.iterdir():
                ...
                # (prefer v2 format when both exist)
                candidate_json = cls._pick_json_file(subdir, file_stem)
            ...
            if not found:
                ...
                fallback_dir = file_stem_subdir / method
                md_file = fallback_dir / f"{file_stem}.md"
                json_file = cls._pick_json_file(fallback_dir, file_stem) or (
                    fallback_dir / f"{file_stem}_content_list.json"
                )
